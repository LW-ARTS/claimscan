...
export function ClaimDialog({
  ...
  const handleClaim = () => execute(tokenMints);
        ...
        <div className="flex flex-col gap-0 px-5 pt-6 pb-5 sm:px-8 sm:pt-8 sm:pb-6" style={{ fontFamily: 'var(--font-sans)' }}>
                  ...
                  {fees.map((fee, i) => {
                    ...
                    return (
                      <div key={fee.id}>
                        ...
                        {i < fees.length - 1 && <div className="h-px w-full bg-[#ddd]" />}
                      </div>
                    );
                  })}
                </div>
          ...
          {phase === 'fetching' && (
            <div className="flex flex-col items-center gap-4 py-12">
              <div className="h-8 w-8 animate-spin rounded-full border-2 border-[#ddd] border-t-black" />
              <p className="text-xs text-[#999]" style={{ fontFamily: 'var(--font-mono)' }}>
          ...
          {phase === 'submitting' && (
            <div className="mt-6 flex flex-col gap-3">
              ...
              <p className="text-center text-[11px] tabular-nums text-[#999]" style={{ fontFamily: 'var(--font-mono)' }}>
                ...
                {results.map((r) => (
                  <div key={r.claimAttemptId || r.tokenMint}>
                    <div className="flex items-center justify-between py-2">
                      <span className="truncate max-w-[120px] text-[11px] text-[#999]" style={{ fontFamily: 'var(--font-mono)' }}>
          ...
          {phase === 'complete' && (
            <div className="mt-6 flex flex-col gap-3">
              {progress.completed > 0 && (
                <div className="flex items-center justify-center gap-2 bg-[#f5f5f5] py-3">
                  <svg className="h-4 w-4 text-black" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
                  </svg>
                  <p className="text-xs font-semibold text-black" style={{ fontFamily: 'var(--font-sans)' }}>
              ...
              {progress.failed > 0 && (
                <div className="bg-[#f5f5f5] p-4">
                  <p className="text-xs text-[#777]" style={{ fontFamily: 'var(--font-mono)' }}>
              ...
              {error && (
                <div className="bg-[#f5f5f5] p-4">
                  <p className="text-xs text-[#777]" style={{ fontFamily: 'var(--font-mono)' }}>{error}</p>
                </div>
              )}
...
function StatusBadge({ status }: { status: string }) {
  const config: Record<string, { text: string; color: string }> = {
    confirmed: { text: 'Confirmed', color: 'text-black font-semibold' },
    failed: { text: 'Failed', color: 'text-[#999]' },
    submitted: { text: 'Confirming...', color: 'text-[#777]' },
    pending: { text: 'Pending', color: 'text-[#ccc]' },
  };
  const c = config[status] ?? config.pending;
  return (
    <span className={`text-[11px] ${c.color}`} style={{ fontFamily: 'var(--font-mono)' }}>
      {c.text}
    </span>
  );
}
