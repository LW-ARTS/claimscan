-- Update get_leaderboard / get_leaderboard_count to include TikTok creators.
-- Replaces the WHERE filter that previously excluded everyone except twitter/github
-- and adds 'tt:' as the handle prefix for TikTok-only creators (matching the 'gh:' pattern).
--
-- Priority order in the COALESCE: twitter > tiktok > github. Twitter stays first
-- to preserve continuity for the existing leaderboard UI; TikTok beats GitHub
-- because TikTok creators are typically more recognizable to a crypto audience
-- than GitHub usernames in the same fee bracket.
