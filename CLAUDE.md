...
- **Flaunch synthetic token ID** (same pattern as Pump): o adapter `lib/platforms/flaunch.ts` retorna UM unico `tokenAddress: 'BASE:flaunch-revenue'` (`tokenSymbol: 'ETH'`) porque `RevenueManager.balances(wallet)` no Base agrega claimable por wallet, nao per-coin. Cache, live overlay e composite key `${platform}:${chain}:${tokenAddress}` usam esse mesmo ID sintetico. Per-coin breakdown fica v2 (exige scan de eventos `RevenueManager.TotalFeesReceived`). Display-only em v1: sem botao de claim, so link externo pra flaunch.gg.
...
- **Marco 2026-04-07 — V2.5 shipped + endorsement do FINN/Bags**: ClaimScan V2.5 lançado (visual redesign, real-time fees, public leaderboard, mobile-first pass, hardening sweep). Cobertura no lancamento: 9 plataformas, 4 chains (Flaunch adicionado depois em v2.6). Métricas no anúncio: $1.7M+ fees tracked, 397+ wallets scanned, ~40% left unclaimed. FINN (@finnbags, founder/CEO da Bags) comentou "nice work!" no post de anúncio (x.com/finnbags/status/2041612548995477746) — validação pública direta do criador de uma das plataformas trackadas. Usar como social proof em outreach pra outros launchpads e em landing copy.
